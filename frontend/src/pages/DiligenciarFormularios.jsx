...
const DiligenciarFormularios = () => {
  ...
  // ✅ Cargar formularios con verificación de array
  const cargarFormularios = async () => {
    try {
      const res = await api.get('mis-formularios/');
      // ✅ Verificar que sea un array
      const data = obtenerDatosSeguro(res.data);
      setFormularios(data);
    } catch (err) {
  ...
  // ✅ Cargar prácticas con verificación de array
  ...
  // ✅ Cargar detalle del formulario con verificación
  ...
  const enviarRespuestas = async () => {
    ...
    // ✅ Verificar que selectedForm exista y tenga campos
    if (!selectedForm || !selectedForm.campos) {
      toast.warning('⚠️ El formulario no tiene campos definidos');
      return;
    }
  ...
  // ============================================================
  // RENDER DETALLE DEL FORMULARIO
  // ============================================================
  if (selectedForm) {
    // ✅ Verificar que selectedForm tenga campos
    const campos = Array.isArray(selectedForm.campos) ? selectedForm.campos : [];
            ...
            <div className="mt-6 space-y-5">
              ...
              <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200">
                ...
                >
                  ...
                  {Array.isArray(practicas) && practicas.map((p) => (
                ...
                <p className="text-xs text-emerald-600 mt-1">
                  Selecciona la práctica a la que corresponde este informe
                </p>
              </div>

              {/* ✅ Campos del formulario con verificación */}
              {campos.length === 0 ? (
                <div className="text-center py-8 text-stone-400">
                  <p>No hay campos definidos para este formulario</p>
                </div>
              ) : (
                campos.sort((a, b) => a.orden - b.orden).map((campo, idx) => (
                  <motion.div 
                    key={campo.id} 
                    ...
                    {campo.tipo === 'checkbox' && (
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input 
                          ...
                          onChange={(e) => handleChange(campo.id, e, campo.tipo)}
  ...
  // ============================================================
  // RENDER LISTA DE FORMULARIOS
  // ============================================================
  const formulariosLista = Array.isArray(formularios) ? formularios : [];
      ...
      <div className="p-6 max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {formulariosLista.length === 0 ? (
            <div className="col-span-2 text-center py-16 bg-stone-50 rounded-2xl border-2 border-dashed border-stone-200">
            ...
            formulariosLista.map((f, idx) => (
              ...
              >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-3">
                      <span className="text-xs text-stone-400 flex items-center gap-1">
                        <span>📊</span> {f.campos?.length || 0} campos
                      </span>
                ...
                <button 
                  onClick={() => cargarDetalleFormulario(f.id)}
